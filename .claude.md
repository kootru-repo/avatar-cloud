...
1. Check if config option needed → add to `config.json`
...
**Customization:** Update paths in `config.json` → `video.sources`
...
6. **Add configuration** in `frontend/config.json` → new feature config
