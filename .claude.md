...
**This applies to:**
- All configuration files (.env, frontend_config.json, backend_config.json)
- All backend code (gemini_client.py, config files)
- All frontend code (gemini-api.js, frontend_config.json)
- All test scripts
- All documentation
...
- ❌ "CLEANUP_SUMMARY.md"
- ❌ Any similar wrap-up documentation

**Why:** The user prefers to track work through git commits and doesn't want summary documents cluttering the project.

**What TO create:**
- ✅ Code files (.js, .py, .html, .css, .json)
- ✅ Configuration files (.env.example, frontend_config.json, backend_config.json)
- ✅ Test files (test_*.py, *_test.js)
- ✅ Essential documentation (README.md updates if critical)

---

## ⚠️ CRITICAL: SDK Usage Policy
...
**⚠️ CRITICAL: Two Different APIs with Different Models**
...
- **ONLY Supported Model**: `gemini-2.0-flash-live-preview-04-09` ✅
...
- ❌ `gemini-3-pro-preview` - Does not exist in Vertex AI
- ❌ `gemini-2.5-flash-native-audio-preview-09-2025` - Google AI Developer API only (not Vertex AI)
- ❌ `gemini-2.0-flash-exp` - Exists in Vertex AI but does NOT support Live API (WebSocket error: "not supported in the live api")
...
frontend/
  ...
  ├── avatar-manager.js            # Avatar video state management
  ├── gemini-live-api.js          # Gemini API client
  ├── live-media-manager.js       # Audio/video capture
  ├── config-loader.js            # Configuration loader
  ├── secure-token-storage.js     # OAuth token storage
  ├── *-manager.js                # Other managers
  └── frontend_config.json         # Frontend configuration

backend/
  ├── main.py                      # WebSocket proxy server
...
All settings centralized in `frontend/frontend_config.json` and `backend/backend_config.json`:
...
**To change settings:** Edit `frontend_config.json` or `backend_config.json`, never hardcode values.
...
2. Backend → Gemini API (with OAuth token)
3. Bidirectional streaming
4. Backend = secure proxy (adds auth, rate limiting)

### Audio Pipeline
- Input: 16kHz PCM16 → Gemini 2.5 Flash Live
- Output: 24kHz PCM16 ← Gemini 2.5 Flash Live
...
1. Check if config option needed → add to `frontend_config.json` or `backend_config.json`
...
1. Add to `frontend_config.json` or `backend_config.json` with sensible default
2. Access via `AppConfig.get('path.to.setting', defaultValue)`
3. Document in code comments if complex

---

## 🚨 CRITICAL DEPLOYMENT WARNING - READ THIS FIRST

### FIREBASE_PROJECT_ID Environment Variable

**⚠️ THIS IS THE #1 RECURRING DEPLOYMENT FAILURE ISSUE ⚠️**
...
**✅ AUTOMATIC DEPLOYMENT (Recommended - Configured and Working):**

A Cloud Build trigger automatically deploys when you push to `main`:
...
2. **Monitor deployment:**
   - Build starts automatically (no manual deploy needed!)
   - View progress: https://console.cloud.google.com/cloud-build/builds?project=avatar-478217
   - Typically takes 3-5 minutes
   - New Cloud Run revision deployed with all env vars and secrets

3. **Trigger configuration:**
   - **Name:** `avatar-backend-trigger`
   - **Source:** kootru-repo/avatar-cloud (main branch)
   - **Build config:** `cloudbuild.yaml`
   - **Service account:** `580499038386-compute@developer.gserviceaccount.com`
   - **Machine type:** E2_HIGHCPU_8
   - **Auto-includes:** FIREBASE_PROJECT_ID, GEMINI_API_KEY secret

**MANUAL DEPLOYMENT (If needed):**

1. **Deploy to Cloud Run manually:**
   ```bash
   cd backend
   gcloud run deploy gemini-avatar-backend --source . --region us-central1 --allow-unauthenticated --min-instances 1 --max-instances 10 --timeout 300 --memory 512Mi --cpu 1 --set-env-vars BACKEND_HOST=0.0.0.0,BACKEND_PORT=8080,DEBUG=false,REQUIRE_AUTH=false,FIREBASE_PROJECT_ID=avatar-478217 --set-secrets GEMINI_API_KEY=GEMINI_API_KEY:latest --port 8080
   ```
...
# Check trigger
gcloud builds triggers describe avatar-backend-trigger --region=us-central1
```
...
Code Changed?
│
├── frontend/* → Run firebase deploy --only hosting
│                If auth fails → firebase login --reauth → wait for user → retry
│
├── backend/*  → AUTOMATIC (Cloud Build trigger on push to main)
│                Just: git push origin main
│                Fallback: Run gcloud run deploy manually (if auth fails, inform user)
│
├── cloudbuild.yaml → Git push (affects next backend auto-deployment)
│
└── Both       → Deploy frontend manually, backend deploys automatically on push
```
...
- Verify `frontend_config.json` loads successfully
- Check Network tab for WebSocket status

### Avatar Video Issues
- Check browser console for video load errors
- Ensure video paths in `frontend_config.json` are correct
...
# PROJECT_ID in frontend_config.json
```
...
**Customization:** Update paths in `frontend_config.json` → `video.sources`
...
3. **Use frontend_config.json and backend_config.json** for all settings
...
## 🆕 SDK Compliance Reference (Nov 2025)
...
### SDK Warnings
⚠️ **Do not interleave `send_client_content` and `send_realtime_input` in the same turn** - can cause unexpected results

---

## 🎯 Function Calling Feature Template: Dance Mode Implementation
...
```
User says "dance"
  → Gemini hears via audio input
  → Gemini calls trigger_dance_mode() function
  → Backend receives tool_call from Gemini
  → Backend forwards to frontend via WebSocket
  → Backend sends tool_response back to Gemini
  → Frontend triggers dance animation + music
  → Gemini continues speaking enthusiastically
  → After 10.084s, dance ends automatically
```
...
if hasattr(response, 'tool_call') and response.tool_call:
    ...
    for function_call in response.tool_call.function_calls:
        ...
        if (session.last_tool_call_name == function_call.name and
            ...
            logger.warning(f"   ⚠️ Skipping duplicate tool call within cooldown")
            should_skip = True
        else:
        ...
        try:
            ...
            logger.info(f"   ✅ Tool call forwarded to frontend")
        except Exception as e:
            logger.error(f"   ❌ Failed to forward tool call: {e}")
        ...
        await session.genai_session.send(tool_response)
        logger.info(f"   ✅ Tool response sent to Gemini (id={function_call.id})")
```
...
triggerDanceMode() {
    ...
    if (!this.danceModeConfig || !this.danceModeConfig.enabled) {
        this.log('❌ Dance mode disabled', 'error');
        return;
    }
    ...
    if (!this.danceModeConfig.duration ||
        ...
        this.log('❌ Invalid dance duration', 'error');
        return;
    }
    ...
    this.danceAudio.addEventListener('error', (e) => {
        const errorMsg = `❌ Dance music failed to load`;
    ...
    this.danceAudio.play().catch(err => {
        console.error('❌ Dance music play failed:', err);
        this.stopDanceMode(false);
    });
...
**File:** `frontend/frontend_config.json`
...
- [ ] User says "dance" → Gemini calls function
...
1. **Define function** in `backend/config/gemini_config.py` → tools array
2. **Add backstory rule** in `backend/whinny_backstory.json` → reaction_imperatives
3. **Implement backend handler** in `backend/core/websocket_handler.py` → tool_call processing
4. **Add session state** in `backend/core/session.py` → debouncing fields
5. **Implement frontend handler** in `frontend/app.js` → tool_call switch case
6. **Add configuration** in `frontend/frontend_config.json` → new feature config
