...
**Architecture:** Vanilla JS frontend → Python backend proxy → Gemini Live API
...
**What TO create:**
- ✅ Code files (.js, .py, .html, .json)
- ✅ Configuration files
- ✅ Test files
- ✅ Essential documentation updates (only if critical)
...
frontend/
  ...
  ├── audio-recorder.js        # Microphone + barge-in
...
backend/
  ...
  ├── config/
  │   ├── gemini_config.py     # Gemini session config
  │   └── prompts.py           # System instructions
  └── core/
      ├── websocket_handler.py # Message handling
      ├── gemini_client.py     # API session
...
- Speaking cycle: forward → reverse/forward loop

### WebSocket Flow
1. Frontend → Backend (WebSocket)
2. Backend → Gemini API (with API key)
3. Bidirectional streaming
4. Backend = secure proxy (adds auth, rate limiting)

### Audio Pipeline
- Input: 16kHz PCM16 → Gemini
- Output: 24kHz PCM16 ← Gemini
...
**IMPORTANT:** Always deploy after making code changes.

### Frontend Changes (index.html, app.js, frontend_config.json, etc.)
```bash
firebase deploy --only hosting
```

### Backend Changes (Python files, backend_config.json, etc.)
```bash
git add -A && git commit -m "<type>: <description>" && git push origin main
```
Cloud Build automatically deploys to Cloud Run on push to main.
...
1. Check if config option needed → add to config file
...
5. Test thoroughly
6. **Deploy changes** (see Auto Deployment above)
7. **DO NOT create summary docs**

### Fixing a Bug
1. Identify root cause
2. Fix the code
3. Test the fix
4. **Deploy changes** (see Auto Deployment above)
5. Commit with clear message
6. **DO NOT create fix summary document**
